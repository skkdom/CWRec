...
#             ipdb.set_trace()
            ...
            if os.path.isfile(v_feat_file_path):
                self.v_feat = torch.from_numpy(np.load(v_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
#                 with open(v_feat_file_path, 'r') as file:
#                     v_data = json.load(file)  # 加载JSON文件内容
    
#     # 假设JSON数据是一个包含浮点数列表的字典，例如：{'features': [0.1, 0.2, ...]}
#                 self.v_feat = torch.tensor(v_data['features'], dtype=torch.float).to(self.device)
            if os.path.isfile(t_feat_file_path):
                self.t_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
#                 with open(t_feat_file_path, 'r') as file:
#                     t_data = json.load(file)  # 加载JSON文件内容
    
    # 假设JSON数据是一个包含浮点数列表的字典，例如：{'features': [0.1, 0.2, ...]}
#                 self.t_feat = torch.tensor(t_data['features'], dtype=torch.float).to(self.device)
