...
#                     save_path = os.path.abspath(os.path.join(self.config['data_path'], self.config['dataset']))
#                     torch.save(self.model.user_embedding.weight.detach(), save_path+str(epoch_idx)+"_f"+"user.pt")
#                     torch.save(self.model.item_id_embedding.weight.detach(), save_path+str(epoch_idx)+"_f"+"item.pt")
# #                     torch.save(self.model.fusion_x.detach(), save_path+str(epoch_idx)+"_2"+"fusion_item.pt")
#                     torch.save(self.model.t_vision.detach(), save_path+str(epoch_idx)+"_f"+"t_vision.pt")
#                     torch.save(self.model.v_vision.detach(), save_path+str(epoch_idx)+"_f"+"v_vision.pt")
#                     torch.save(self.model.sample_fusion_x.detach(), save_path+str(epoch_idx)+"_f"+"sample_fusion_x.pt")
# #                     torch.save(self.model.v_vision.detach(), save_path+str(epoch_idx)+"_2"+"sample_x.pt")
# #                     torch.save(self.model.sample_fusion_x.detach(), save_path+str(epoch_idx)+"_2"+"pred_x.pt")
#                     torch.save(self.model.mm_x.detach(), save_path+str(epoch_idx)+"_f"+"mm_x.pt")
...
#                     torch.save(self.model.state_dict(), model_save_path)
                  # 保存最佳模型权重
                    if saved:
                        checkpoint_dir = self.config['checkpoint_dir'] if 'checkpoint_dir' in self.config else 'saved'
                        ...
                        if verbose:
                            self.logger.info(f'模型权重已保存到: {model_save_path}')
